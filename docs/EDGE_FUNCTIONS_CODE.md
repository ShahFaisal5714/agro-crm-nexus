# Edge Functions for Personal Supabase Deployment
...
4. Create each function folder and file as shown below
5. Deploy: `supabase functions deploy FUNCTION_NAME`
...
serve(async (req) => {
  ...
  try {
    ...
    if (password.length < 8 || !/[A-Z]/.test(password) || !/[a-z]/.test(password) || !/[0-9]/.test(password)) {
      return new Response(
        JSON.stringify({ error: "Password must be 8+ chars with uppercase, lowercase, and number" }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    const { error: roleInsertError } = await supabaseAdmin
      .from("user_roles")
      .update({ role, territory: territory || null })
      .eq("user_id", authData.user.id);

    if (roleInsertError) {
      // Fallback: insert if update failed (no existing role)
...
serve(async (req) => {
  ...
  try {
    ...
    const { userId, role, territory } = await req.json();
...
## 4. reset-user-password/index.ts
...
After creating the functions, you may want to set JWT verification. Create or update `supabase/config.toml`:
...
[functions.change-user-email]
verify_jwt = false
```

Note: We set `verify_jwt = false` because we manually verify the JWT in the function code using `getClaims()` pattern for flexibility.
