...
export const CreateInvoiceFromCreditsDialog = ({
  ...
  // Prepare invoice items from credits
  const invoiceItems = credits.map(credit => ({
    product_id: credit.product_id || "",
    quantity: 1,
    unit_price: credit.amount,
    description: credit.description || `Credit from ${format(new Date(credit.credit_date), "MMM dd, yyyy")}`,
  }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Build invoice notes with credit history
      const creditSummary = credits.map(c => 
        `${format(new Date(c.credit_date), "MMM dd, yyyy")}: ${formatCurrency(c.amount)} - ${c.description || "Credit"}`
      ).join("\n");
      
      const paymentSummary = payments.map(p => 
        `${format(new Date(p.payment_date), "MMM dd, yyyy")}: ${formatCurrency(p.amount)} - ${p.payment_method}`
      ).join("\n");

      const fullNotes = `
Dealer Credit Invoice for ${dealerName}
...
${formData.notes ? `Additional Notes: ${formData.notes}` : ""}
      ...
      if (items.length === 0 && credits.length > 0) {
